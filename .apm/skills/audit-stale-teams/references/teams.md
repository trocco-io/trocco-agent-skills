...
まず `trocco-api.md`、関連して `resource-groups.md` を読むこと。
...
- リソースグループが `teams: [{ team_id, role }]` を持つ（`resource-groups.md`）。
...
2. 統合する場合:
   ...
   - 参照しているリソースグループがあれば、`teams` を残すチームへ付け替え（`resource-groups.md`）。
